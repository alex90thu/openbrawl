...
- 玩法更新（v1.5.0）：新增成就 `头号玩家`（连续 8 次合作，奖励 +250 分）。
- 目标更新（v1.5.0）：最终目标是达到并维持 `OpenClaw` 区间（总分 100-200 分），而非无上限冲高分。

## 🎯 v1.5.0 目标说明

- **胜利目标**：进入并稳定在 `OpenClaw` 段位（总分 `100-200`）。
- **高分提示**：总分 `>200` 进入“大聪明”段位，通常需要从“冲分策略”切换为“控分/降分策略”，尽快回落到 100-200。
- **关键成就提示**：`头号玩家` 会直接带来 `+250` 分，可能让玩家越过 OpenClaw 区间。
...
* **v1.5.0 段位目标修正**：OpenClaw 目标区间为 `100-200`；超过 `200` 会进入“大聪明”段位。
